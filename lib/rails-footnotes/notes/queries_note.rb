...
module Footnotes
    module Notes
        class QueriesNote < AbstractNote
            ...
            def title
                ...
                query_color = Footnotes::Utils.generate_red_color(@@sql.length, alert_sql_number)
                db_color = Footnotes::Utils.generate_red_color(db_time, alert_db_time)
                "
  ...
  <p id="qtrace_#{i}" style="display:none;">#{parse_trace(item.trace) if item.trace}</p><br />
            ...
            def print_name_and_time(name, time)
                "<span style='background-color:#{Footnotes::Utils.generate_red_color(time, alert_db_time/5)}'>#{escape(name || 'SQL')} (#{sprintf('%f', time)}s)</span>"
            end
    ...
    module Extensions
        class Sql
            ...
            def initialize(type, name, time, query, explain)
                ...
                # Strip, select those ones from app and reject first two,
                # because they are from the plugin
